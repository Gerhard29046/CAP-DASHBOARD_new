...
| Forms | Mobile-first audit of every form (one-column layout, labels, large touch targets, keyboard behavior) | **DONE** — `a8f665d`. Global primitive fix: Input/Select/Button step up to ~44px touch targets on mobile (md: back to desktop density), Checkbox/Switch get an invisible 44px hit area via `.tap-target`. 2 real bugs found+fixed: `ProductsServicesSettings.jsx` dialog's Type/SKU row was still bare `grid-cols-2` in a `max-w-md` dialog (375px-cramped, same class already fixed elsewhere 2026-08-13); phone inputs in `ClientDetail.jsx`/`CompanySettingsPanel.jsx` missing `type="tel"`. Everything else audited (MachineForm/ServiceForm/ClientForm/BookIn sectioning/most grids) was already correct from the 2026-08-13 redesign, left unchanged. `npm run lint`/`typecheck` clean, `npm test` 76/76, `npm run build` clean. Not live-clicked. |
| Modals/drawers | Convert appropriate desktop modals/drawers to bottom sheets / full-screen sheets on mobile | **DONE** — `bace0dc`. Primitive fix: `Dialog` (shared by every form dialog in the app) renders as a bottom sheet below `sm:` (anchored bottom, rounded top, slide-up, safe-area-aware, drag-handle affordance, built-in `max-h`/scroll safety net), unchanged centered card at `sm:`+/desktop. Real bug fixed as a side effect: `ProductsServicesSettings.jsx`'s product/service dialog had no `max-h`/`overflow-y-auto` of its own, so a tall form could render off-screen with unreachable Save/Cancel buttons — now covered by the primitive default. `AlertDialog` (confirmations) deliberately kept as a centered card, not converted to a sheet (short/1-2 buttons, spec treats sheet-conversion as "where appropriate" not mandatory) but gained edge margin + consistent rounding + overflow safety net. `PhotoLightbox.jsx` opted back out of the new sheet default (full-screen photo viewer, not a form). No app-level side-drawers/detail-panels exist to convert (`Sheet` primitive is only used by Phase 1's `MobileBottomNav`/an unrelated shadcn `sidebar.jsx`). `npm run lint`/`typecheck` clean, `npm test` 76/76, `npm run build` clean. Not live-clicked; one dialog's nested-scroll interaction (`StickyNotes.jsx`'s `ClientPicker`) flagged as not specifically device-tested. |
| Tables | Transform desktop tables into mobile card/list rows where appropriate | **DONE** — `e4cf9e3`. Audit found `Clients.jsx`/`Jobs.jsx`/`ServiceRecords.jsx` already dual-render table+card (2026-08-13 work); `UserAdmin.jsx` already card-based; `InvoiceQueue.jsx`/`ImportCustomers.jsx`'s remaining raw tables are genuinely tabular (financial line items; admin CSV batch-review with inline controls) with proper scroll already — left as-is, a reasoned "appropriate to scroll" call per spec section 14. Real fix made: `ServiceRecords.jsx`'s bespoke photo-preview overlay (no focus trap/Escape/safe-area) replaced with the shared `PhotoLightbox` component already used by `MachineDetail.jsx`/`JobCardDetail.jsx`. `npm run lint`/`typecheck` clean, `npm test` 76/76, `npm run build` clean. Not live-clicked. |
| Detail pages | Mobile-first redesign of client/machine/job-card/service-record detail screens | **DONE** — `db46d7c`. `ClientDetail.jsx`/`MachineDetail.jsx`/`JobCardDetail.jsx` audited (Service Records has no dedicated detail page, edited inline in `ServiceRecords.jsx`/`MachineDetail.jsx`, already covered by earlier phases); all three already reflowed well from the 2026-08-13 redesign. 5 real, isolated touch-target/reachability bugs found+fixed (each confirmed via grep to be the only occurrence, not systemic): `MachineDetail.jsx`'s per-service-record Edit/Delete icon buttons hardcoded `h-8 w-8` overriding Phase 2's 44px mobile Button default at every breakpoint; `JobCardDetail.jsx`'s per-line-item Edit/Delete were raw `<button>`s with `p-1` (~24px hit area), given `.tap-target`; `JobCardDetail.jsx`'s header action row had no mobile stack/wrap fallback (unlike every other page's `PageHeader`), real overflow risk at 320-375px; `JobCardDetail.jsx`'s status quick-select chips bumped from `py-1.5` (~28px) to `py-2.5`/`md:py-1.5`; `ClientDetail.jsx` + `StickyNotes.jsx` delete-note buttons used `opacity-0 group-hover:opacity-100`, completely unreachable on touch devices (no hover state) — now always visible below `md:`, hover-reveal only at `md:`+. `npm run lint`/`typecheck` clean, `npm test` 76/76, `npm run build` clean. Not live-clicked, not yet pushed/deployed. |
...
| 9 | Settings (Android; web's hub already exists, extend it) | **DONE**, real-build-verified (overnight 2026-08-16). New Settings hub (`android-ui-bee`) mirroring web's tabs as mobile-appropriate sections: Job Cards config editor, Products & Services catalogue CRUD, honest empty-state copy for General/System/Data Management (no CSV import on mobile this phase), Users & Roles link-out. Gated end-to-end on `settings.access`, matching web's RoleGuard exactly. Found (by `android-ui-bee`, before shipping) and fixed (by `supabase-android-bee`) a real data-layer blast-radius bug: `SupabaseData.kt`'s `fetchAll()` hardcoded `order=created_at.desc` for every table; `job_card_settings` has no such column and would have 400'd on cold start, which (via `combine()`) would have broken every OTHER screen's data too, not just Settings — fixed with a small `TABLES_WITHOUT_CREATED_AT` denylist, live-confirmed against production (400→200) by `testing-bee`. Gradle: `BUILD SUCCESSFUL` (clean build, one real compile bug found+fixed along the way — a nested Kotlin block comment), 16/16 unit tests (corrects a stale "23" baseline cited mid-session — 16 is the actual current baseline post-Phase-12 Firebase removal), 0 lint errors/28 warnings (+1, a disclosed/accepted tradeoff), real 21,750,440-byte APK, zero Firebase strings. **Read AND write paths live-verified, including the UX-honesty fix (2026-08-17): 38/38.** `qa-verify-phase9-settings-rls.mjs`'s first real run (29/32) found that a denied `products_services`/`job_card_settings` UPDATE/DELETE returned HTTP 204 (indistinguishable from success) rather than an error, since those are `USING`-clause RLS policies and `SupabaseData.kt` didn't check affected-row-count. Fixed same session, per explicit user go-ahead: `update()`/`delete()` now request the row representation and throw if it comes back empty (same message an outright 403 already used). Re-verified live at 38/38 (script grew from 32→38 checks to also prove the converse — a genuine write, including a no-op edit, is never false-denied). Incidentally found, unrelated, disclosed: an `anon`-role default-grant gap on some post-`0002` tables (see `KNOWN_ISSUES.md`, no active data exposure). |
| 10 | Theming/personalization (web AND Android — neither has this today) | **PARTIAL** — Android done, web not started. Android: real Light/Dark/Follow-System toggle (Settings > Appearance), replacing the app's previous single hardcoded dark scheme. New `CapLightColorScheme` (role-for-role mirror of the dark scheme, sourced from the website's own light `:root` CSS tokens), `CapThemeController`/`CapThemePreferences` (plain `SharedPreferences`, no new Gradle dependency), system-bar icon appearance switching, status-tone colors (`CapStatus.kt`) made scheme-aware. Two disclosed deliberate deviations from a literal 1:1 color port (brand `primary` held constant across both schemes rather than following the website's light-mode darkening; `secondary` NOT held constant, since it's a dark elevated-surface tone on this app, not a brand accent) — both reasoned and documented in `Theme.kt`. Real-build-verified alongside Phase 9 (same commit/session). **On-device visual behavior is unverified** — compiles and builds correctly, nobody has seen the light scheme rendered on a real screen. Web-side dark mode explicitly out of scope this pass (separate, larger, cross-file Tailwind initiative) — not started. |
| 11 | Connection & Sync Status → Supabase-based, real checks | **DONE**, real-build-verified — `2eb9f33`. `checkHealth()`/`testConnection()`/`sync()`'s `apiHealthy` used to probe a now-stale Firestore `users/{uid}` doc (accurate before `b8aaaee`, actively wrong after — could show "Connected" during a real Supabase outage, or "broken" while Supabase was fine). Now uses `SupabaseAuthRepository.hasSession` + `SupabaseDataRepository.count("permissions")` (same primitive `sync()` already used elsewhere), no FirebaseAuth/FirebaseFirestore dependency left in `StatusRepository` at all. UI text updated (no more "Firebase"/"Firestore"/`capdatabasefb2` labels). Implemented directly by Queen Bee (repository-layer work, `supabase-android-bee` not invocable this session). Gradle: `BUILD SUCCESSFUL`, 23/23 tests, 0 lint errors/30 warnings (unchanged baselines), real APK |
