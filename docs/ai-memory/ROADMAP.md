...
| 12 | Firebase removed completely from Android | NOT DONE — footprint now much smaller than originally scoped (see below); `observeFirestoreCollection()` (Core.kt) is confirmed unreachable dead code as of `b8aaaee`, and `StatusRepository`'s Firebase dependency is now gone too (Phase 11, above). What's left: delete `observeFirestoreCollection()` itself, the Firebase Auth login bridge (`AuthRepository`'s best-effort secondary sign-in, `Core.kt` ~line 450+ — it existed solely to satisfy `firestore.rules` for the old `users` read, which no longer happens), and the Firebase Gradle dependencies themselves, then prove it (grep the built APK/dependency tree for any remaining `firebase-*`/`com.google.firebase` reference) |
