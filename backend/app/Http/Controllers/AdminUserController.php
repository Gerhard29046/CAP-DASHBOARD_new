...
class AdminUserController extends Controller
{
    ...
    {
        $data=$r->validate(['name'=>[$user?'sometimes':'required','string','max:255'],'email'=>[$user?'sometimes':'required','email',Rule::unique('users')->ignore($user)],'password'=>[$user?'sometimes':'required','string','min:8','confirmed'],'role'=>[$user?'sometimes':'required',Rule::in(['admin','technician','accountant','custom'])],'is_active'=>['sometimes','boolean'],'permissions'=>['sometimes','array'],'permissions.*'=>['boolean']],['email.unique'=>'This email address is already in use.','password.confirmed'=>'Password confirmation does not match.','role.in'=>'The selected primary role is invalid.','permissions.*.boolean'=>'One or more selected permissions are invalid.']);
