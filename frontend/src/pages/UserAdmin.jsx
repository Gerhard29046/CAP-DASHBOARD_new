    ...
    <div className="grid lg:grid-cols-[300px_1fr] gap-5">
      ...
      {(selected || matrix.length > 0) && <form onSubmit={save} className="space-y-4">
        <div className="bg-card border rounded-2xl p-5 grid md:grid-cols-2 gap-3">
          {Object.keys(FIELD_LABELS).map((key) => <div key={key}><Label>{FIELD_LABELS[key]}</Label><Input type={key.includes("password") ? "password" : "text"} autoComplete={key.includes("password") ? "new-password" : undefined} value={form[key] || ""} onChange={(e) => setForm({ ...form, [key]: e.target.value })} required={!selected && Object.keys(FIELD_LABELS).includes(key)} />{key === "password" && <p className="mt-1 text-xs text-muted-foreground">Use 8+ characters with uppercase, lowercase, a number, and a symbol.</p>}{fieldErrors[key]?.map((error) => <p key={error} className="mt-1 text-xs text-destructive">{error}</p>)}</div>)}
