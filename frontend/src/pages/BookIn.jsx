...
export default function BookIn() {
  ...
  // Job cards use a human-readable sequential number.
  // The first new job is JOB-0001, then JOB-0002, JOB-0003, etc.
  //
  // Existing legacy/random numbers such as JOB-684645 or UUID-based values
  // are deliberately ignored when calculating the next number so they cannot
  // make the new numbering random.
  const getNextJobNumber = async (prefix = "JOB-") => {
    const jobs = await apiClient.entities.JobCard.list();

    const escapedPrefix = prefix.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
    const pattern = new RegExp(`^${escapedPrefix}(\\d{4})$`, "i");

    const numbers = (jobs || [])
      .map((job) => String(job?.job_number || "").trim().match(pattern))
      .filter(Boolean)
  ...
  // guarded by `!jobCardId` so it can't double-create on a re-render.
  //
  // The sequential job number is assigned at creation time so the record never exists
  // without its real human-readable number. This is important because photos are uploaded
  // against this early-created record.
  useEffect(() => {
    if (selectedMachineId && client?.id && !jobCardId) {
      ...
      const createJobCard = async () => {
        try {
          const settings = await apiClient.entities.JobCardSettings.get().catch(() => null);
          const prefix = settings?.numbering_prefix || "JOB-";

          setJobCardSettings(settings);

          const createWithNextNumber = async (attempt = 0) => {
            const jobNumber = await getNextJobNumber(prefix);

            try {
              const created = await apiClient.entities.JobCard.create({
              ...
              // If another user created the same number between the list and insert,
              // calculate again and retry once.
              //
              // Other errors are allowed to surface normally and are NOT retried,
              // so we don't accidentally create duplicate job cards.
              const message = String(error?.message || error || "").toLowerCase();

              const isDuplicate =
                message.includes("duplicate") ||
                message.includes("unique") ||
                message.includes("job_number");

              if (attempt === 0 && isDuplicate) {
                return createWithNextNumber(1);
              }
          ...
          const { created, jobNumber } = await createWithNextNumber();

          if (cancelled) return;

          setJobCardId(created.id);

          setForm((prev) => ({
            ...prev,
            job_number: created.job_number || jobNumber,
          }));
        } catch (error) {
          if (!cancelled) {
            console.error("Could not create sequential job card:", error);
            setUploadError(
              "Could not start this job card. Photos cannot be added right now."
            );
          }
        }
      };
  ...
  // Load Job Card settings without generating a timestamp/random job number.
  // The number is assigned by the early job-card creation above.
  useEffect(() => {
    apiClient.entities.JobCardSettings.get()
      .then((settings) => setJobCardSettings(settings))
  ...
  useEffect(() => {
    if (presetMachineId) {
      apiClient.entities.Machine.get(presetMachineId).then(async (m) => {
        ...
        const jobs = await apiClient.entities.JobCard.filter(
          { machine_id: m.id },
          "-date_received"
        );
  ...
  const handleSelectClient = async (c) => {
    ...
    const machines = await apiClient.entities.Machine.filter({
      client_id: c.id,
    });
  ...
  const handleUploadPhoto = async (e) => {
    ...
    if (!jobCardId) {
      setUploadError(
        "This job card hasn't been created yet. Please try again."
      );
      e.target.value = "";
      return;
    }
    ...
    try {
      const path = await uploadRecordPhoto(
        RECORD_PHOTO_NAMESPACES.jobCard,
        jobCardId,
        file
      );
      ...
      await apiClient.entities.JobCard.update(jobCardId, {
        arrival_photos: next,
      });
    } catch (err) {
      setUploadError(
  ...
  const removePhoto = async (path) => {
    ...
    if (jobCardId) {
      try {
        ...
        setUploadError(
          err?.message || "Could not remove that photo. Please try again."
        );
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // Photos are already persisted incrementally (see handleUploadPhoto) --
      // finalize the remaining fields via update(), not a second create().
      const card = await apiClient.entities.JobCard.update(jobCardId, {
        ...
        // BUG FIX (2026-08-13): the "Machine Type" input has always existed
        // on this form and InvoiceQueue.jsx has always displayed it, but
        // job_cards had no such column and this payload never included it.
    ...
    return (
      <div className="max-w-lg mx-auto">
        ...
        <div className="mb-5">
          ...
          <p className="text-sm text-muted-foreground mt-0.5">
            {selectedClient.company_name} — Select a machine
          </p>
        </div>
    ...
    <div className="max-w-lg mx-auto">
        ...
        <div className="bg-card border border-border rounded-2xl p-5 space-y-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              ...
              <Input
                value={form.job_number || "Generating…"}
                readOnly
            ...
            <div>
              ...
              <Input
                type="date"
                value={form.date_booked_in}
                onChange={e => set("date_booked_in", e.target.value)}
                required
        ...
        <div className="bg-card border border-border rounded-2xl p-5 space-y-4">
          ...
          <div>
            ...
            <Textarea
              ...
              placeholder="Describe the fault or reason for booking in…"
        ...
        <div className="bg-card border border-border rounded-2xl p-5 space-y-4">
          ...
          <div>
            ...
            <Select
              ...
              onValueChange={v => set("condition_on_arrival", v)}
            >
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-2 mt-2">
              {photos.map((path, i) => (
                <div
                  key={path + i}
                  className="relative w-20 h-20 rounded-xl overflow-hidden bg-secondary border border-border"
                >
                  {photoUrls[path] ? (
                    <img
                      src={photoUrls[path]}
      ...
      <div className="mt-6 bg-card border border-border rounded-2xl p-5">
        <h2 className="font-heading font-semibold text-sm text-foreground flex items-center gap-2 mb-4">
          ...
          Previous Jobs for This Machine
        </h2>

        {previousJobs.length === 0 ? (
          <p className="text-sm text-muted-foreground text-center py-4">
            No previous job cards found
          </p>
        ) : (
                ...
                <div className="min-w-0">
                  ...
                  <p className="text-xs text-muted-foreground truncate">
                    ...
                    {job.fault_description
                      ? ` · ${job.fault_description.slice(0, 50)}${
                          job.fault_description.length > 50 ? "…" : ""
                        }`
                      : ""}
                  </p>
                </div>
